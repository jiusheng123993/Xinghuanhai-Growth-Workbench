...
## 2026-09-19 · ZYZ-AI指令合集评估结论落地（提示词链路补强 + 名字清洗缺口修复 + 积压入库）

**时间**：2026-09-19 凌晨 ｜ **编号**：无（舰长 + 4 施工 + 2 复核 + 1 重核）

**一、评估（已完成的判断，结论见 `E:\Codex\2026-09-19\zyz-video-prompts\outputs\ZYZ资料包-视频生图可用性评估.html`）**
- 对象：`E:\BaiduNetdiskDownload\ZYZ-AI指令合集2026`（16.2 GB / 4755 文件），按用户口径**只评估「视频生成 + 生图」相关**。
- 相关文档 **214 个，覆盖 214/214（零遗漏）**；转换管道：markitdown 177 + Word COM 98 + pypdf 19。
  - ⚠️ 踩坑：Word COM 复用实例处理完第一个文档就自杀（报误导性的 null-valued expression，真因是 RPC 0x800706BA）⇒ 改一文件一实例；
    另 `SaveAs(7)` 实际写 GBK 却被按 UTF-16 读回 ⇒ `.doc/.rtf` 全乱码，由 1 号船员报回后改读 `Content.Text` 并全量重生成。
- 结论：**有帮助但是「小补丁」级**。真正可采纳约一成，其中三条：① 我们缺「出板→当参考图→出视频」的**中间参考图层**；
  ② 几条我们真没有的写法（运镜位移速度、镜头级七字段 schema、负面描述独立成段、静→动→静）；
  ③ 反向确认我们的十段结构比它的 Seedance 2.0 模板（只有 4 段 / 6 段）更完备。
- **独立复核（审-1）**：10 条关键论断 **成立 2 / 部分成立 8 / 完全不成立 0**，无一条是读错文件，8 条是「口径说过头」。
  被**证实**的：神兽卡的 52%/30%/18% 与「华丽度 4.5/5」是**模板常量**（200 只里出现 199 次）⇒ 数值不可照抄。
  被**证伪**的：「资料包里宠物专有信号一条都没有」——实际 `胡须`25、`尾巴`109、`瞳孔`307，口径已改。
- 清洗后重核（6 号）：板家族**规模被低估**（不是 3 张板，而是 ≥6 套配对）；抽查 10 条引文 4 条与原文不符（细节级，不推翻结论）。

**二、本轮落地的改动（提交 `34f06a5` + 队长接线）**
- `promptTemplates.ts`：注释补全 **5 把锁**（此前只写 3 把，实现早已 5 把 —— 典型注释漂移）；
  `Shot` 默认段补**位移速度**（原资料包措辞「2 秒/帧」字面反常，队长裁决改写为「整段位移约 2 秒内匀速完成」）与**静→动→静**；
  `PHYSICS` 默认段补 **6 类宠物专有信号**（耳位/胡须/尾尖/竖毛/舔鼻/瞬膜）——这是我们的护城河，A02+A08 两包内 5/7 项 0 命中；
  `IDENTITY LOCK` 吸收「三元色彩占比 + 参考图当量级基准」两条**形式**（不写具体数值）。
- `promptTemplates.test.ts`：新增「锁条数与锁名齐全」守卫，防注释再漂移。
- `petPrompt.ts`：新增 `stripNamingPhrases()` —— **句式级**命名清洗，拦 **LLM 自己编造**的第三方名字
  （白名单 `translatePetNames` 对这类完全无能，是「烧鸡」事故的同型风险）。
- **队长接线（关键）**：2 号受任务书限制只改了 2 个文件，新函数**没接任何调用点 = 死代码**。
  队长按「在共用函数加守卫才不会漏掉兄弟调用方」把它接进 `translatePetNames` 内部（先句式清洗、后白名单替换），
  一处改动覆盖全部 5 个调用点（`routes/avatar.ts:298,405`、`familyPhotoService.ts:436`、`memoirScriptService.ts:297,301`）。
- `宠物回忆录-提示词库.md`：追加 9 章（十一~十九，+379 行）；收敛两处锁文案与代码逐字对齐；给「秒/帧」加口径警告。
- `.dsh/skills/pet-prompt-engine/SKILL.md`：同步两道清洗、宠物专有信号、避坑清单。

**三、验证（全部真跑）**
- `npx vitest run promptTemplates.test.ts petPrompt.test.ts` → **64 passed（12 + 52）**，退出码 0。
- `npx tsc --noEmit`（server）、miniapp `tsc --noEmit` → 均退出码 0。
- **负向测试（防假绿）**：把 `translatePetNames` 里的接线临时摘掉 → 新断言**精确报红**
  （`expected '一只叫小橘的猫在窗台上' not to contain '小橘'`）⇒ 断言不是装饰；随后已还原并复跑全绿，确认无 `TEMP-NEGATIVE-TEST` 残留。
- 文档体检：2048 行、U+FFFD 0。

**四、积压入库（用户指示「先把未提交提交了」）**
- 工作区原有 **152 项**未提交改动（09-11~09-12 舰队批次 + 文档 + 设计素材），按目录分 3 笔入库：
  `ae7cedf` miniapp（141 文件）、`cafc313` server（10 文件）、`cb43391` 文档与设计素材（534 文件）。
- ⚠️ 提交前已确认**没有 codex 在本仓改动**（当时 3 个 codex 全在 `E:\会金小厨房`），且两侧 tsc 全绿才提交 ——
  这条是本项目血的教训（曾把半截补丁提交进去，HEAD 编译不过一整晚）。
- **刻意未提交**（判定为临时产物，仍留在工作区未跟踪）：`__scan6.js`、`ts-scan.js`、`index.tsx`、`AGENTS.md.bak-slim`。
- ⚠️ `cb43391` 里夹带了 `02-UI设计/首页优化预览/` 下的**一次性迁移脚本与 backup 副本**（`append-progress-*.js`、`fix-*.js`、`*.bak-*`、`.svg-cache/`），
  属"文档纪律"里该清理的一类；已向用户报备，**未擅自删除**（怕误删其有意保留的备份）。

**五、遗留与下一步（未做，不要当成已完成）**
- ⭐ **「出板」中间参考图层未落地**：设计文档早就有「模式 C（先专用关键帧再视频）」与 `0.3.1 多角度参考包`，
  但**代码只吃 1 张图（用户原照片）**。资料包给出了经过多题材验证的工序，是本次唯一够格算"方向性"的发现。
  属产品可见变化（多一次出图调用 = 成本与时长），**留待用户拍板**，不擅自实施。
  ⚠️ 另发现 `Codex暂存/回忆录关键帧/` 已有 s01~s09 九张关键帧（已随 `cb43391` 入库），可作为该工序的现成起点。
- 宠物专有信号目前只在 `PHYSICS` **默认段**（LLM 缺段时才生效），尚未做"每次强制注入"——要不要提升为强制，待定。
- `stripNamingPhrases` 已知边界：名字后**无标点**时会吃掉后文（`名字是旺财今天很乖` → `这只狗`）；补语黑名单仅 21 条。
- 资料包全部自称适配 **Seedance 2.0**，而那批数值参数（f/5.6、5000K、+20%、暗角 -15%）在 **2.5** 下是否等效**未验证**。
- 案例里常用 `2.35:1 宽银幕 + 15 秒`，我们是小程序**竖屏**——画幅与时长口径**待用户定**。
---

## 2026-09-19 · 星河宠记：画幅定 16:9 + 落地「模式 C」（每镜先出关键帧）+ 接入四视图设定图参考（提交 `91cf5c0`）

**用户拍板**：① 画幅要 **16:9**（原为跟随照片的 `adaptive`）；② **出板要做**，且按「**每一镜都先出关键帧**」落地。

**接手侦察纠正了我上一轮的说法**（重要）：
- **真正的「残缺模块」比我先前讲的更具体**：`avatar_multiview_url`（四视图全身设定图）**早就存在**，
  `familyPhotoService.ts:284` 里已按「真实照片 > 设定图 > 卡通头像」在用，**唯独回忆录链路只传单张真实照片**
  （`videoGenerationService.ts:375`）。⇒ 注释声明了、全家福实现了、**回忆录没接**。
- 画幅现状是 `ratio: 'adaptive'`（跟随照片，导致每段画幅不一致、拼接观感参差）。

**服务端（1 号施工 + 队长复核修正）**
- 🆕 `memoirKeyframeService.ts`：每镜先用 Seedream **多图图生图**出一张 **16:9 电影质感静帧**；
  参考图 = 该镜真实照片（画面依据：姿态取静/构图取准/场景取真）+ **四视图设定图**（可空，只核对毛色花纹体型五官，
  且**显式禁止复制其分格版式**——不写禁令模型会照抄成四宫格拼图）；提示词整段复用该镜十段 prompt，只补「这是静帧」「画幅 16:9」；
  失败一律 `return null`，绝不抛异常给主链路。
- `videoGenerationService.ts`：关键帧成功 → 作 Seedance 首帧；失败 → **回落原照片**；新增 `MEMOIR_SEGMENT_RATIO='16:9'` 常量，三条生成路径一次性对齐。
- `imageBadge.ts`：`addAiBadge` 新增 `{ visible }` 选项（**默认 true，既有 4 处调用点行为不变**）。

**前端（2 号施工）**：`memoir-daily` / `memoir-full` 比例盒 `padding-top: 75% → 56.25%`；`yearly-review` 播放器按 16:9 定高（`360rpx → 395rpx`，容器内容宽 702rpx 反算）；三处页面注释同步。

**三个我拍板的裁决（都是"会改变可见行为"的点）**
1. **关键帧不烙可见角标** → `addAiBadge(url, {visible:false})`：只跳过合成，**隐式 AIGC 元数据照写**。
   理由：角标会被 Seedance **动起来**（可能扭曲成渲染缺陷），且等于给成片**凭空增加一个用户可见元素**。
2. **真 API 实测 `size='1920x1080'`**（1 号无法联网验证）：HTTP 200、返回真实图片 URL ⇒ 风险关闭。
   背景：项目其它三处**一律只用过 `1024x1024`**，若该 size 被拒，关键帧会**静默逐镜回落**（功能等于没上）。
3. **样例片重制**：实测 `sample-light-1` = 960×720（4:3）、`sample-light-2` = **720×960（3:4 竖版）**，
   塞进 16:9 盒会左右大黑边。已本地用 ffmpeg（模糊背景填充）重制为 **1280×720** 两片，交付在
   `E:\Codex\2026-09-19\zyz-video-prompts\outputs\`，**待部署替换**；未替前样例预览会有黑边。

**验证（全部真跑）**
- 全量后端测试 **88 文件 / 1353 测试全通过**；server 与 miniapp `tsc --noEmit` 均 **0**。
- **真实渲染量尺**（CDP 挂常驻 Edge :9222，视口 375 自检通过）：
  `memoir-daily .memoir__hero-frame` = **343.03×192.95px → 比例 1.7778**；`memoir-full .memoir-vlog__sample-frame` 同为 **1.7778** ⇒ 16:9 成立。
- `yearly-review` 无数据分支未渲染（count=0，按技能"先怀疑数据不怀疑样式"），改用**代理渲染**量尺：
  H5 量到 **2.252:1** —— 与 `miniapp-ui-polish §二.7` 记录的既有陷阱**数值一致**（H5 下 `rpx→rem` 固定根字号 16px，
  与百分比宽换算系数不同）⇒ **该处 H5 不是有效判据，真机反算 702rpx/395rpx = 1.7772 = 16:9**。
- **产物核验**（防 §二.9 静默漏样式表）：`dist-h5` 里 `56.25%` 命中 2 处、`padding-top:75%` 归零、页面独有类均在。

**成本影响**：每镜多一次 Seedream 调用 ⇒ 约 **+2.4~3.2 元/条**（对照整条 Seedance 13~20 元，约 +12%~25%），出片 +1~2 分钟（关键帧串行）。

**遗留**
- 「**空间调度板**」（俯视平面图 + 机位编号 C1-C4 + 移动路径）文档与代码**都还没有** —— 本轮只落地了模式 C 的关键帧，不是调度板。
- **样例片需部署替换**（重制文件已产出）；未替前用户看到的样例预览会有黑边。
- **关键帧未做人眼视觉验收**（四宫格是否漏出 / 外貌一致性 / 静帧是否真静）—— 建议首条真片抽看。
- 年度回顾的播放盒是 **rpx 定高**，真机才成立；若要更稳可日后改百分比盒（本轮因类名直接挂在单标签 `<Video>` 上、无内部子元素，未改）。
---

## 2026-09-19 · 【决策留痕】回忆录成片不做可见水印/角标（用户拍板）

**决策**：**回忆录成片不加任何可见水印或 AI 角标。** 用户 2026-09-19 在被告知下述合规风险后明确选择「不做水印」。

**当前实现基线（已逐项核实，非推断）**
- 成片（视频）链路**无任何可见叠加** —— 全仓搜 `drawtext|drawbox|overlay=|logo|watermark` 无命中；
  唯一的 `ass=filename=...`（`videoGenerationService.ts:606`）是**旁白字幕**，属产品功能，不是水印。
- 上游平台水印**已关**：Seedream/Seedance 请求体一律 `watermark: false`
  （`image2DService` / `familyPhotoService` / `avatarService` / `videoGenerationService` / `memoirKeyframeService`）。
- 关键帧（中间产物）**不烙角标**：`addAiBadge(generatedUrl, { visible: false })`。
- **隐式 AIGC 标识保留**：成片 mp4 元数据 `stampAigcVideoMetadataOnFile()`（`memoirProcessor.ts:213`）；
  PNG 走 `appendAigcPngMetadata()`。二者依据《标识办法》第十条（下载服务的元数据标识义务）。
- ⚠️ **图片侧角标未动**：全家福 / 头像 / 表情包仍带「AI 绘制 · 星河宠记」角标（本次决策**不覆盖**它们）。

**已知风险（用户知情后决定，务必保留此段）**
- **2026-04-28，网信部门依法查处「剪映」App、「猫箱」App 及「即梦AI」网站**，事由为
  **「未有效落实人工智能生成合成内容标识规定要求等问题，违反《网络安全法》」**。
  来源：新华网 / 腾讯新闻 / 新浪科技 / 网信办官网（多源一致，距决策日不足 5 个月）。
- 该判例说明：**「用户能不能关掉标识」本身就是执法关注点**；"同行也这么做"（如即梦会员去水印）
  **不构成依据** —— 它恰恰是被查处的对象。
- 我方风险落点：面向公众的**微信小程序**；是否触发以**平台审核与监管口径**为准（**未做法务结论**）。
- 缓解项：隐式元数据标识**已落地**；若日后需要显式标识，**做成片级**
  （在 ffmpeg 合成那一步统一烙一次：位置稳定、不会被 Seedance 动、只加一处），
  **不要做成关键帧级**，也不要下放成用户可关闭的开关。

**决策边界**：仅覆盖**回忆录成片**。图片侧角标、以及成片的旁白字幕均不在本次决策内。
> ⚠️ **本条已被同日后续决定扩展**（见下一条 2026-09-19「决策留痕·扩展」）：
> 图片侧（全家福/头像/形象/表情包）**也已去掉可见角标**，并覆盖了 2026-09-09 的旧红线
> 「AI 角标不可付费去除」。**单看本条会得出"图片侧还有角标"的过时结论，请两条一起读。**
---

## 2026-09-19 · 【决策留痕·扩展】图片侧也去掉可见角标（覆盖 2026-09-09 的旧红线）

**决定**：**图片侧（全家福 / 头像 / 形象 / 表情包）也去掉可见 AI 角标。** 至此**全链路无可见角标**
（图片侧 + 回忆录成片 + 关键帧）。用户在同一天内两次拍板：先「成片不做水印」，再「图片侧也去」。

**⚠️ 本次明确覆盖了一条项目既有的红线**（必须留痕，不是悄悄改）：
- 被覆盖的原文：`01-产品文档/回忆录2.0-素材体系完整设计-2026-09-09.md:152`
  「**AI 角标不可付费去除**（会员差异化不得是"去水印"，合规红线）；走 addAiBadge 转存本站管线。」
- 处置：该条已就地标注为「**已被 2026-09-19 决定取代、不再生效**」，保留原文供沿革，并附新决定 + 风险依据 + 保留项。
- 意义：去角标这件事**项目以前考虑过并明确否决过**；本次是**知情后的反转**，不是遗漏。

**实现（提交待定，工作区已验证）**
- `addAiBadge` → **重命名为 `hostAiImage`**：职责如实描述为「把临时 CDN 图转存本站 + 写隐式元数据」。
  留一个叫 `addAiBadge` 却在多数情况下不加角标的函数是名不副实，后来人会误判。
- 选项 `{ visible?: boolean }` → `{ withBadge?: boolean }`，**默认 false**（不合成）。
  角标通道**保留为一行开关**（`withBadge: true`），当前**无任何调用点**开启 —— 属日后可能需要的唯一入口。
- 4 个调用点：`avatarService` / `familyPhotoService` / `image2DService`（去掉角标）/ `memoirKeyframeService`
  （顺带去掉那个别扭的 `{ visible: false }`）。
- ⭐ **隐式 AIGC 元数据一条未丢**：PNG `appendAigcPngMetadata` / MP4 `stampAigcVideoMetadataOnFile` 照旧。
  它在文件元数据里、**用户看不见**，不是"水印"，是本轮刻意**保留**的合规底线（沿用《标识办法》第十条口径）。
- 连带修：`aigcMetadata.ts` 文件头注释原本写「显式标识已由 imageBadge.ts 落地」，已失真，改为如实描述
  「只做隐式、不做显式」。

**验证（真跑，我的 shell，默认池）**
- `npx tsc --noEmit` → **0**；全量 `npx vitest run` → **88 文件 / 1353 测试全通过**。
- 施工船员另做了**变异对照**：把默认值临时改回「默认合成」→ 新用例如期变红（证明断言不是装饰）。
- 旧名残留：`src` 下零引用（仅 `imageBadge.ts` 两行散文式改名说明，刻意保留）。

**文档同步（避免"代码改了、文档还在说反话"）**
- `宠物回忆录-提示词库.md` §19.2：改为「全链路不做可见角标」+ 重命名说明 + 保留项 + 已知风险 + 恢复方式。
- `01-产品文档/立项/资料/local-inventory.md`：两处（形象库行、内容合规行）改为当前状态。
- `回忆录2.0-素材体系完整设计-2026-09-09.md`：旧红线标注取代。
- 未改：立项书 v0.1/v0.2、market-research、路径轴、商业事实台账等**带日期的快照类文档** —— 它们描述的是当时状态，
  改它们等于改写历史；**权威口径以本文件（progress.md）的决策留痕为准**。

**已知风险（与上一条同源，用户知情后决定）**：2026-04-28 网信部门查处「剪映」「猫箱」「即梦AI」，
事由「未有效落实人工智能生成合成内容标识规定要求，违反《网络安全法》」。**未做法务结论**，
以平台审核与监管口径为准。恢复路径：`hostAiImage(url, { withBadge: true })` 一行，或按需做成片级/整图级统一烙一次。