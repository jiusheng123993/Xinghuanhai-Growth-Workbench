...
const DEFAULT_SECTIONS: Record<string, string> = {
  ...
  //   ① 运镜位移速度量化：给出「整段位移约 2 秒内匀速完成」，让运镜节奏可判定，而不是「缓慢」这类主观词。
  //      ⚠️ 资料包原措辞是「速度 2 秒/帧」，字面义为「每帧推 2 秒」，对视频模型是反常单位；
  //      这里只保留它的**意图**（给出可判定的时长），改写成不会被误读的表述。
  ...
  'Shot':
    '缓慢推镜，从原始景别匀速向主体面部收拢，整段位移约 2 秒内匀速完成，无卡顿、无骤停；整体节奏：静→动→静，起幅静置、中段小幅微动、收幅回到静置；主体保持原始姿态，缓慢眨眼并伴随轻微呼吸，随后耳朵或尾巴尖做小幅自然动作；只使用一种运镜。',
